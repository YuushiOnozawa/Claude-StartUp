...
1. `sync-whitelist.conf` から include パスを読む
2. 各 include パスについて `rsync --dry-run -av --checksum` または `diff -rq` で比較
3. カテゴリ（新規/変更/同一）に分類して出力
4. `known-deletions.conf`（後述）に載っているファイルは「削除予定（既知）」カテゴリで別出力
...
| 状態 | 判定 |
|---|---|
| `sync-whitelist.conf` が存在しない | fail（中断） |
| 実働環境パスが存在しない | fail（中断） |
