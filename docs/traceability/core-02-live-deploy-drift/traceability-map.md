...
|---|---|---|---|
| SPEC-02-01（de-git） | TEST-02-01-01, TEST-02-01-02 | 手動確認 | **PENDING**（de-git 未実施） |
| SPEC-02-02（whitelist） | TEST-02-02-01〜03 | 自動/verify | **PASS** |
| SPEC-02-03（sync-check.sh） | TEST-02-03-01〜11（scripts/test-sync-check.sh 流用） | 自動/CI/verify | **9 PASS / 0 FAIL / 1 SKIP** |
...
|---|---|---|---|---|
| A-001 | MEDIUM | 受入条件「初回の還流棚卸し + MM ファイル取捨判断の記録」に SPEC/IMPL/TEST なし | valid | **対象外**（手動実施済み扱い・requirements.md に注記追加） |
