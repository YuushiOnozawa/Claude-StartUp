...
**対象箇所（2026-07-14 更新。PR #289 の execution-steps.md 大規模書き換えにより行番号・箇所数が変化したため現状に合わせて改訂）:**

| ファイル | 変更前 | 変更後 |
|---|---|---|
| `skills/magi-common/references/execution-steps.md` L153 | `bash scripts/magi-diff-filter.sh` | `bash "$HOME/.claude/scripts/magi-diff-filter.sh"` |
| `skills/magi-common/references/execution-steps.md` L186 | `bash scripts/magi-split-hunk.sh 400` | `bash "$HOME/.claude/scripts/magi-split-hunk.sh" 400` |
| `skills/magi-hard/SKILL.md` L134 | `bash scripts/magi-impact-context.sh` | `bash "$HOME/.claude/scripts/magi-impact-context.sh"` |

> 旧対象のうち `magi-fast/SKILL.md` L36・`magi-hard/SKILL.md` L34 の `magi-diff-filter.sh` 呼び出しは
> PR #289 で execution-steps.md 側へ集約され消滅した（旧5箇所 → 現3箇所）。

**変更しないもの:**

- `execution-steps.md` L268-269: `bash scripts/ollama-check.sh ... || bash ~/.claude/scripts/ollama-check.sh`
  （リポジトリ内実行時は repo 版優先・他 cwd では絶対パスへフォールバックする二段構え。既に cwd 非依存のため対象外）
- `execution-steps.md` L379: `bash ~/.claude/scripts/ollama-run.sh`（既に絶対パス）

### 事後条件

- 対象3箇所全てが `"$HOME/.claude/scripts/..."` 形式になっている
- 他プロジェクト cwdで MAGI スキルを実行しても、スクリプトが見つかる

### 検証方法

```bash
# 変更後の確認（"bash scripts/" が残っていないこと。ollama-check のフォールバック一段目は対象外）
...
| 状態 | 判定 |
|---|---|
| 対象ファイルに `bash scripts/` が残っている（L268 のフォールバック一段目を除く） | fail |
...
| スクリプト相対パスの絶対パス修正（3箇所。2026-07-14 改訂） | 自動（実装 PR） | SPEC-03.1-04 |
