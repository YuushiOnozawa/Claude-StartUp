...
> **注記（2026-07-19 PR-D 実装時・人間確認済み）**: `lessons-learned-distill.sh` の knowledge-rag 登録は
> transcript から自動抽出したファイルのみが対象であり、手動保存ファイルを走査・登録する経路は存在しない
> （Codex 設計レビュー指摘）。**手動保存はローカル記録のみ**とし、登録経路の追加は本 SPEC のスコープ外と
> することを人間判断で確定した。CLAUDE.md にもその旨を明記した（PR #324）。
...
| pCloud 配送処理 | 対象外 — `scripts/pcloud-sync.sh`（core-01 impl）へ委任。hook は関与しない（2026-07-19 Step 7 で旧方針行を修正） | SPEC-03.2-03 |
...
| pCloud 配送処理 | 対象外 — `scripts/pcloud-sync.sh`（core-01 impl）へ委任。hook は関与しない（2026-07-19 Step 7 で旧方針行を修正） | SPEC-03.2-05 |
...
| 4 | knowledge-rag 登録先が MCP サーバー側 `documents_dir`（live = `~/pcloud/obsidian`、setup/402 生成）に依存し、REQ-03.2-03 の「mount なしで登録・自動昇格が完走」が未達（Step 7 レビュー HIGH）。documents_dir 再設計・「完走」の定義確定は [#326](https://github.com/YuushiOnozawa/Claude-StartUp/issues/326) で別トラック管理（2026-07-19 人間確認） |
