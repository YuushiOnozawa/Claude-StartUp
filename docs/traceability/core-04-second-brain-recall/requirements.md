...
- **vault 内の人間向けフォルダ体系設計**（第2の脳としての整理・MOC・タグ・アーカイブ規約等）は
  本 core 群のスコープ外。全 core 消化後に別途設計する（ユーザー決定 2026-07-08）。
  SPEC-04-03 で確定した機械ゾーン（`distilled/`・`knowledge/`・`vault/_inbox/` 等の少数固定フォルダ）
  はこの対象外設計に含まれない。人間ゾーンの再編は機械ゾーンに触れない限り自由
