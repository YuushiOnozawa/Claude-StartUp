...
for persona in melchior balthasar casper; do
  PERSONA_NAME=$(printf '%s' "$persona" | tr '[:lower:]' '[:upper:]')
  MAGI_PLAN_RECEIPT=
  if [ "$persona" = casper ]; then
    MAGI_PLAN_RECEIPT=$(cat "$RUN_DIR/plan-receipt.json" 2>/dev/null || true)
    if [ "${#MAGI_PLAN_RECEIPT}" -gt 4096 ]; then
      MAGI_PLAN_RECEIPT=''
    fi
  fi
  ...
  MAGI_CHANGE_SUMMARY="${MAGI_CHANGE_SUMMARY:-}" \
  MAGI_PLAN_RECEIPT="${MAGI_PLAN_RECEIPT:-}" \
  run_persona "$persona"
done
```
