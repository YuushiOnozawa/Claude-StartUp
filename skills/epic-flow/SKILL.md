...
| 規模 | 目安 | ルーティング |
|------|------|------------|
| **DEV** | 単一機能・影響ファイル 1〜3 件 | `/dev-flow` に委譲 |
| **EPIC** | 複数機能・影響ファイル 4 件以上・独立した PR が複数想定される | EPIC フローを続行 |
...
作成した Issue 番号を `$ISSUE_LIST`（例: `[42, 43, 44]`）として保持する。
作成完了後、Issue 一覧をユーザーに提示する。
