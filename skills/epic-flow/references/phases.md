...
**分岐 3: `status: active`**
→ 引数 `#M` が明示指定されている場合はこの確認をスキップし、直接「再開（続きから）」として処理する
→ state の `epic:` フィールドを読み取り、AskUserQuestion で確認:
  question: "アクティブな Epic が見つかりました（`<epic フィールドの内容>`）。続きから再開しますか？"
  options: ["再開（続きから）", "新規開始（state を退避して新しい Epic を開始）", "この依頼は別件（state を維持したまま Phase 0 へ進む）"]
  「新規開始」の場合: 旧 state を `epic-state-<YYYYMMDD>.md` にリネームして退避 → SCALE ASSESSMENT（SKILL.md Phase 0）に戻る
  「別件」の場合: state には触れず SCALE ASSESSMENT（SKILL.md Phase 0）に進む（後で `/epic-flow` 再入時に旧 Epic をそのまま再開できる）
