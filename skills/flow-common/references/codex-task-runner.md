...
- read-only モード: stdout への Codex 応答テキスト
...
| `artifact` | `(cd "$TASK_TMPDIR" && node "$CODEX_COMPANION" task "..." --write)` | 生成物を tmpdir に書き出す |
| `repo-write` | `(cd "$WORKTREE_PATH" && node "$CODEX_COMPANION" task "..." --write)` | コード実装（codegen のみ）|
...
**read-only（既定）:**
```bash
node "$CODEX_COMPANION" task "$(cat "$TASK_TMPDIR/task-prompt.txt")" \
  > "$TASK_TMPDIR/task-raw.txt" 2>/dev/null
```

**artifact:**
```bash
(cd "$TASK_TMPDIR" && node "$CODEX_COMPANION" task "$(cat "$TASK_TMPDIR/task-prompt.txt")" --write) \
  > "$TASK_TMPDIR/task-raw.txt" 2>/dev/null
```

**repo-write（codegen のみ）:**
```bash
(cd "$WORKTREE_PATH" && node "$CODEX_COMPANION" task "$(cat "$TASK_TMPDIR/task-prompt.txt")" --write) \
  > "$TASK_TMPDIR/task-raw.txt" 2>/dev/null
```
