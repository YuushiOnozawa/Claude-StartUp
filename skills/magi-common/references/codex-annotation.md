...
`magi-hard` も出力権限は annotation JSON のみである。`--write`、ファイル編集、Git の状態変更、network access の要求は禁止する。

hard に限り、呼び出し元が用意した隔離済み snapshot 内の**tracked regular files のみ**を調査対象としてよい。profile は `codex-companion-read-only/v1` とし、Codex CLI availability、companion plugin path、`--write` を渡さないコード経路、isolated root の realpath、snapshot manifest、実行前後の tree hash を呼び出し元が `$RUN_DIR/audit/profile-verification.json` に記録する。network 遮断は Codex companion 経由では検証不能であり、`network_isolation` フィールドとして常に `not_supported_by_codex_companion_1.0.5` を記録し、判定根拠にしない。

`-C` は workspace の指定であって、ホスト側へのアクセスを遮断するものではない。呼び出し元（F6 実装）は `$RUN_DIR/audit/profile-verification.json` の `annotation_eligible` が true の場合だけ annotation 経路を実行する。この profile が検証できない環境では、本 contract の `hard` annotation 経路を実行してはならない。
...
これは共通ランナーの read-only 形式に `-C "$ISOLATED_ROOT"` を追加した本 contract 固有の変形であり、`--write` は付けない。呼び出し元は起動前に `$ISOLATED_ROOT` として `git archive` 由来の tracked snapshot を用意し、tree hash/manifest を保存してから上記 profile で起動する責務を負う。

prompt は isolated root 内の関連 tracked files に限定し、root 外・untracked・`.git`・credential・`$HOME`・`/proc`・network 等の探索を要求しない。snapshot 内で読めない場合は `needs_human` または注釈省略とし、権限を拡大しない。
