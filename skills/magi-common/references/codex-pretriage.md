...
`codex-annotation.md` の hard read-only contract と同じ executor profile を必須とする。network と write を無効化し、tracked regular files だけの read-only snapshot を `$ISOLATED_ROOT` とする。`.git`、untracked、symlink 越境、credential、`$HOME`、`/proc`、環境変数探索、shell/Git/ネットワークは拒否する。`-C` や prompt の禁止文は境界ではない。この profile がない環境では Codex を実行せず、`fallback_legacy` を選ぶ。

prompt file と raw response は private `audit/pretriage/` に user-only permission で置く。executor は timeout、stdout byte limit、引数配列（shell なし）を持つ。CLI で executor を明示する場合も `--isolated-profile hard-read-only/v1` がなければ実行せず `fallback_legacy` とする。timeout、non-zero、size overflow は response schema failure と混同せず audit の構造化 error category に記録する。raw response を会話 context、renderer、診断へ転載しない。
