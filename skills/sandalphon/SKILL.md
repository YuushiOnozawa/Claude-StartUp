...
3. プロンプトを一時ファイル経由で Ollama に渡す（特殊文字対策 + 排他ロック）：
   ...
   # プロンプトを一時ファイルに書き出し
   printf '%s' "$PROMPT" > /tmp/magi-lfm-prompt.txt
   # タイムアウト付きロック取得（最大5分）
   flock -w 300 /tmp/magi-ollama.lock ollama run lfm2.5:8b < /tmp/magi-lfm-prompt.txt || {
     echo "⚠ Ollama ロック取得タイムアウト（5分）。他のプロセスが実行中か確認してください。"
     rm -f /tmp/magi-lfm-prompt.txt; exit 1
   }
   rm /tmp/magi-lfm-prompt.txt
   ```
