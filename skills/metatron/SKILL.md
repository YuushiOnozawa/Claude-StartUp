...
```bash
ollama list 2>/dev/null | grep -q "devstral:latest"
```
...
1. Read ツールで以下を読み込む（repo 内を優先、なければ絶対パスで `~/.claude/` を使用）：
   ...
   - `skills/metatron/references/task-instruction.md`（repo 内）または `/home/<user>/.claude/skills/metatron/references/task-instruction.md`
...
3. プロンプトを Ollama に渡す（特殊文字対策: printf + pipe）：
   ```bash
   printf '%s' "$PROMPT" | bash ~/.claude/scripts/ollama-run.sh devstral:latest || {
...
「⚠ Ollama が利用できません（モデル `devstral:latest` が見つかりません）。Claude Haiku にフォールバックしてよいですか？」
...
- `skills/metatron/references/task-instruction.md` の内容（ロール定義・few-shot例）
