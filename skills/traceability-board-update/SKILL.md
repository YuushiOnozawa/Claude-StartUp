...
4. **既存ボードの「実行順」「運用メモ」セクションは人間の決定記録なので、削除せず内容そのまま
   再生成後のファイルに含める**（集約で作り直すのはステータス表・次のアクション候補・blocked のみ）
5. core README とボードの食い違いを検出した場合は core README を正として直し、差分を報告する
