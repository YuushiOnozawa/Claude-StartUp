...
また `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1` を設定し、adaptive thinking を無効化している。`effortLevel: high` と adaptive thinking の併用で CLAUDE.md ルールやスキル定義が無視される挙動（[claude-code#23936](https://github.com/anthropics/claude-code/issues/23936)）の回避が目的であり、モデル非依存の設定。
