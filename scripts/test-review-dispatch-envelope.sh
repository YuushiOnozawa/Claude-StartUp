...
write_envelope "$CASE_FILE" hard codex failed indeterminate false null true post_failed "" "" "dispatch handoff missing"
expect_exit "hard・failed + post_failed（未投稿保証・canonical codex）を受理する" 0 "$CASE_FILE"
write_envelope "$CASE_FILE" hard magi failed indeterminate false null true posted "" "" "review-post exit 1"
jq '.native_result = {"github_writes": []}' "$CASE_FILE" > "$TEST_ROOT/r5-empty-writes.json"
expect_exit "hard・failed + posted + native_result.github_writes=[]（API試行後は post_failed にしない）を受理する" 0 "$TEST_ROOT/r5-empty-writes.json"
